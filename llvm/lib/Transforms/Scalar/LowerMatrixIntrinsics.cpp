...
public:
  ...
  /// Special case for MatMul lowering. Prevents scalar loads of row-major vectors
  /// Lowers to vector reduction add instead of sequential add if reassocation is enabled.
  void lowerDotProduct(CallInst *MatMul,
    ...
    InstructionCost SequentialAddCost =
      TTI.getArithmeticInstrCost(AddOpCode, Add->getType()) *
      (LShape.NumColumns - 1);
    
    if (ReductionCost >= SequentialAddCost)
      return;

    // Lambda functions as dyn_cast<BuiltinLoad>, with correct stride
    auto GetBuiltinLoad = [](Value *Val, int N) -> CallInst * {
      ...
      if (match(Val, m_Intrinsic<Intrinsic::matrix_column_major_load>(
        ...
        LoadStride->getZExtValue() == N) {
        return cast<CallInst>(Val);
      }
      return nullptr;
    };

    CallInst *LHSBuiltinLoad = GetBuiltinLoad(LHS, LShape.NumColumns);
    CallInst *RHSBuiltinLoad = GetBuiltinLoad(RHS, RShape.NumColumns);
    
    // Matmul must be the only user of loads because we don't use LowerLoad for row vectors
    // (LowerLoad results in scalar loads and shufflevectors instead of single vector load).
    if (match(LHS, m_CombineOr(m_Load(m_Value()),
    ...
    if (match(RHS, m_CombineOr(m_Load(m_Value()),
                               m_Intrinsic<Intrinsic::matrix_column_major_load>()))) {
      if (RHS->hasOneUse())
    ...
    Value *Mul = IsIntVec ? Builder.CreateMul(LHS, RHS)
    ...
    if (IsIntVec)
      ...
      Result = Builder.CreateFAddReduce(ConstantFP::get(
                       cast<VectorType>(LHS->getType())->getElementType(), 0.0),
                   Mul);
