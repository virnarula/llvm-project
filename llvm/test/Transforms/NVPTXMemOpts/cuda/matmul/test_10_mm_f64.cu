...
__global__ void matrixMultiplyNaive(const double *A, const double *B, double *C, int width) {
...
__global__ void matrixMultiplyCoalesced(const double *A, const double *B, double *C, int width) {
    __shared__ double sharedA[16];
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void matrixMultiplyCoalescedPrefetched(const double *A, const double *B, double *C, int width) {
    __shared__ double sharedA[16];
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;

    double sum = 0;
    double tmp = A[idy * width + threadIdx.x];
    for (int i = 0; i < width; i += 16) {
...
int main() {
    ...
    double *h_C_prefetched = new double[N * N];  
