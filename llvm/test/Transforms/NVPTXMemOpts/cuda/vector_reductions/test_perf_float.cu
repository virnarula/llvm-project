...
__global__ void naive_reduction(float *input, float *output, int row, int cols) {
...
__global__ void coalesced_prefetched(float *input, float *output, int row, int cols) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (idx < cols) {
        ...
        // Shared memory for coalesced access
        float tmp = input[threadIdx.x * cols + idx];
        for (int i = 0; i < row; i += 16) {
            __shared__ float shared[16];
...
int main() {
    ...
    size_t bytes_input = N * M * sizeof(float);
    size_t bytes_output = M * sizeof(float);

    float *h_input = new float[N * M];
    float *h_output = new float[M];
    float *h_output_coalesced = new float[M];
    float *h_output_coalesced_prefetched = new float[M];
