...
__global__ void vectorScalarMultiply(const double *input, double *output, double scalar, int numElements) {
...
// CUDA kernel for vector scalar multiplication
__global__ void vectorScalarMultiply_coalesced(const double *input, double *output, double scalar, int numElements) {
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    __shared__ double input_shared[16];
