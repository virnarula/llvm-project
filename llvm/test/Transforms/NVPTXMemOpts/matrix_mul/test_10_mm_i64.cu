...
__global__ void matrixMultiplyNaive(const int64_t *A, const int64_t *B, int64_t *C, int width) {
...
__global__ void matrixMultiplyCoalesced(const int64_t *A, const int64_t *B, int64_t *C, int width) {
    __shared__ int64_t sharedA[16];
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void matrixMultiplyCoalescedPrefetched(const int64_t *A, const int64_t *B, int64_t *C, int width) {
    __shared__ int64_t sharedA[16];
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;

    int64_t sum = 0;
    int64_t tmp = A[idy * width + threadIdx.x];
...
int main() {
    ...
    int64_t *h_C_prefetched = new int64_t[N * N];
